$(document).ready(function(){
	...
	// LEFT FIXED
	function LFixed(){
		if($(window).width() > 980 && $(window).height() > $('.left-fixed').outerHeight(true)+$('footer').outerHeight(true) && $('.left-fixed').length){
			...
			$(window).scroll(function(){
				if($(window).width() > 980 && $(window).height() > $('.left-fixed').height()+$('footer').height()) setCond();
			});
		}
		else $('.left-fixed').removeClass('fixed');
	}
	...
	$('input.print-price').click(function(){
		...
		prt_window.document.write('<html><head><title>Прайс-лист</title>');
	...
	//Order form
	var OF_TA_sel = 'Выбрано: '+$('#content h1').text()+'\nДополнительная информация:\n';
